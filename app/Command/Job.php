...
class Job
{
    ...
    {
        $users = User::where('quantitypay','1')->get();
        foreach ($users as $user) {
            if ($user->money >= GoPassConfig::get('quantit_money') && $user->transfer_enable-($user->u+$user->d) <= '100') 
            {
                $user->money -= GoPassConfig::get('quantit_money');
                $user->transfer_enable += '1073741824';
                $user->save();
            }/*else{
                $subject = Config::get('appName') . '-按量扣费失败';
                $to = $user->email;
                $text = '您好，系统为您按量扣费时，发现您的余额不足，请充值后使用。';
                try {
                    Mail::send($to, $subject, 'news/warn.tpl', [
                        'user' => $user,
                        'text' => $text
