...
class UserController extends BaseController
{
    public function index($request, $response, $args)
    {
        ...
        return $this->view()
            ...
            ->assign('time', date("Y-m-d H:i:s", time()))
    ...
    public function code($request, $response, $args)
    {
        ...
        return $this->view()->assign('class_left_days', floor((strtotime($this->user->class_expire) - time()) / 86400) + 1)->assign('class_left_days', floor((strtotime($this->user->class_expire) - time()) / 86400) + 1)->assign('time', date("Y-m-d H:i:s", time()))->assign('shops', $shops)->assign('codes', $codes)->assign('pmw', Payment::purchaseHTML())->assign('bitpay', BitPayment::purchaseHTML())->assign('time', date("Y-m-d H:i:s", time()))->display('user/code.tpl');
    }
    ...
    public function agentbuy($request, $response, $args)
    {
        ...
        if (bccomp($user->money, $price, 2) == -1) {
            $res['ret'] = 0;
            $res['msg'] = '当前余额不足，总价为' . $price . '元。请充值~！';
            return $response->getBody()->write(json_encode($res));
        } else {
    ...
    public function node($request, $response, $args)
    {
        ...
        $nodes = Node::where('type', 1)->orderBy('node_class')->orderBy('sort2', 'asc')->get();
    ...
    public function announcement($request, $response, $args)
    {
        ...
        return $this->view()->assign('class_left_days', floor((strtotime($this->user->class_expire) - time()) / 86400) + 1)->assign('time', date("Y-m-d H:i:s", time()))->assign('anns', $Anns)->display('user/announcement.tpl');
    }
    ...
    public function invite($request, $response, $args)
    {
        ...
        return $this->view()->assign('ref_money', $ref_money)->assign('invite_user', $invite_user)->assign('class_left_days', floor((strtotime($this->user->class_expire) - time()) / 86400) + 1)->assign('time', date("Y-m-d H:i:s", time()))->assign('code', $code)->assign('paybacks', $paybacks)->assign('paybacks_sum', $paybacks_sum)->display('user/invite.tpl');
    }
    ...
    public function shop($request, $response, $args)
    {
        ...
        $shop_price1 = GoPassConfig::get("shops_price")['plan_1'];
        $shop_price2 = GoPassConfig::get("shops_price")['plan_2'];
        $shop_price3 = GoPassConfig::get("shops_price")['plan_3'];
        $shop_price4 = GoPassConfig::get("shops_price")['plan_4'];
        $shops_free_price_d = GoPassConfig::get("shops_free_price")['plan_free'];
        ...
        $shops_free_price_diy = Shop::where(
            static function ($query) use ($shops_free_price_d) {
                $query->Where('id', '=', $shops_free_price_d['d_price_1'])
                ->orwhere('id', '=', $shops_free_price_d['d_price_2'])
                ->orwhere('id', '=', $shops_free_price_d['d_price_3'])
                ->orwhere('id', '=', $shops_free_price_d['d_price_4'])
                ->orwhere('id', '=', $shops_free_price_d['d_price_5'])
                ->orwhere('id', '=', $shops_free_price_d['d_price_6'])
                ->orwhere('id', '=', $shops_free_price_d['d_price_7'])
                ->orwhere('id', '=', $shops_free_price_d['d_price_8'])
                ->orwhere('id', '=', $shops_free_price_d['d_price_9']);
            }
        )->where('status', '=', 1)->orderBy('price', 'asc')->get();
        ...
        return $this->view()
            ...
            ->assign('shops_free_price_diy', $shops_free_price_diy)
            ->assign('time', date("Y-m-d H:i:s", time()))
            ->assign('shops', $shops)
    ...
    public function ticket($request, $response, $args)
    {
        ...
        return $this->view()->assign('tickets', $tickets)->assign('time', date("Y-m-d H:i:s", time()))->display('user/ticket.tpl');
    }

    public function ticket_create($request, $response, $args)
    {
        return $this->view()->display('user/tickcreate.tpl');
    }
    ...
    public function ticket_view($request, $response, $args)
    {
        ...
        $ticketset = Ticket::where('id', $id)->orWhere('rootid', '=', $id)->orderBy('datetime', 'asc')->paginate(5, ['*'], 'page', $pageNum);
        $ticketset->setPath('/user/ticket/' . $id . '/view');
  
        return $this->view()->assign('ticketset', $ticketset)->assign('time', date("Y-m-d H:i:s", time()))->assign('ticket_status', $ticket_main->status)->assign('id', $id)->display('user/ticket_view.tpl');
    }
    ...
    public function updatename($request, $response, $args)
    {
    ...
    public function doCheckIn($request, $response, $args)
    {
        ...
        if ($this->user->quantitypay == '1') {
            $res['ret'] = 0;
            $res['msg'] = '您已启用按量付费，签到无效';
            return $response->getBody()->write(json_encode($res));
        }
    ...
    public function detect_index($request, $response, $args)
    {
        ...
        return $this->view()->assign('rules', $logs)->assign('time', date("Y-m-d H:i:s", time()))->display('user/detect_index.tpl');
    }
    ...
    public function detect_log($request, $response, $args)
    {
        ...
        return $this->view()->assign('logs', $logs)->assign('time', date("Y-m-d H:i:s", time()))->display('user/detect_log.tpl');
    }
    ...
    public function buyTrafficPackage($request, $response, $args)
    {
        ...
        $result = GoPassConfig::get('shop_traffic_package') * $traffic;
    ...
    //按量付费
    public function quantitypay($request, $response, $args)
    {
        $user = $this->user;
        $bought = Bought::where('userid', $user->id)->first();
        if ($bought && strtotime($user->class_expire) > time() && $user->quantitypay == '0') {

            if ($user->money < Config::get('quantit_money')) {
                $res['ret'] = 0;
                $res['msg'] = "余额不足，请先充值";
                return $response->getBody()->write(json_encode($res));
            }
            if (floor((strtotime($user->class_expire) - time()) / 86400) + 1 <= 2) {
                $res['ret'] = 0;
                $res['msg'] = "会员时长不足2天,无法启用";
                return $response->getBody()->write(json_encode($res));
            }
            self::Quantit(1);
            $res['ret'] = 1;
            ...
            $res['ret'] = 0;
            $res['msg'] = "您的账号未购买过正常套餐，由于安全原因，暂时不能启用按量付费功能。";
            return $response->getBody()->write(json_encode($res));
        }
    }

    public function Quantit($set)
    {
        $user = $this->user;
        if ($set == 1) {
            $user->class = GoPassConfig::get('quantit_class');
            $user->t = '0';
            $user->u = '0';
            $user->d = '0';
            $user->transfer_enable = '1073741824';
            $user->class_expire = '2039-12-30 00:00:00';
            $user->node_speedlimit = GoPassConfig::get('quantit_speedlimit');
            $user->node_connector = GoPassConfig::get('quantit_connector');
            $user->quantitypay = '1';
            $user->save();
        } elseif ($set == 0) {
            $user->class = '0';
    ...
    public function subscribe_log($request, $response, $args)
    {
        ...
        return $this->view()->assign('logs', $logs)->assign('iplocation', $iplocation)->assign('time', date("Y-m-d H:i:s", time()))->display('user/subscribe_log.tpl');
    }
